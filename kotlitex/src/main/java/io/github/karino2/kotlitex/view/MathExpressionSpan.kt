...
private class MathExpressionDrawable(expr: String, baseSize: Float, val fontLoader: FontLoader, val isMathMode: Boolean, val drawBounds: Boolean = false, equationColor: Int) {
    ...
    val equationcolor = equationColor
...
class MathExpressionSpan(val expr: String, val baseHeight: Float, val assetManager: AssetManager, val isMathMode: Boolean, val eqColor: Int) : ReplacementSpan() {
    enum class Align {
        Bottom, BaseLine
    }
    ...
    private fun getDrawable(): MathExpressionDrawable {
        ...
        val drawable = MathExpressionDrawable(
            expr, baseHeight,
            AndroidFontLoader(assetManager), isMathMode, drawBounds = false, equationColor = eqColor
        )
        return drawable
    }
